...
double calc_E(int Lattice[len][len][len], int i, int j, int k)
{
    ...
    Energy = -1.0*J*Lattice[i][j][k]*(Lattice[left][j][k]+Lattice[right][j][k]+Lattice[i][up][k]+Lattice[i][down][k]+Lattice[i][j][towards]+Lattice[i][j][away]);
    if (Energy == -0)
    {return 0;}
    else
    {return Energy;}
...
void flip_spin(int (&Lattice)[len][len][len], int i, int j, int k, double T, double &dE)
{
    ...
    Lattice[i][j][k] = -1*Lattice[i][j][k]; //flip the spin
    double Ef = calc_E(Lattice, i, j, k); //calculate the new energy
    Lattice[i][j][k] = -1*Lattice[i][j][k]; //flip the spin back
    dE = Ef-E0;
    //cout << "dE = " << dE << endl;
    double r = ((double)rand())/((double)RAND_MAX);
    if (Ef <= E0)
...
void write_to_file(vector<double> &mc_E, vector<double> &temp_vec)
{
    ...
    string fname = "/Users/Casey/Dropbox/REU Project 2014 Casey/Codes/mc_ising3D_data.txt";
